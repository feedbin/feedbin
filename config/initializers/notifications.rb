ActiveSupport::Notifications.subscribe("process_action.action_controller") do |name, start, finish, id, payload|
  if payload[:params]["controller"] && payload[:params]["action"]
    ...
    if finish && start
      ...
      Librato.timing "controller.#{controller}.#{action}.time", time, percentile: [95]
      if payload[:params].present? && payload[:params]["subdomain"].present? && payload[:params]["subdomain"] == "api"
        Librato.timing "response_time.api", time, source: Socket.gethostname, percentile: [95]
      else
        Librato.timing "response_time.web", time, source: Socket.gethostname, percentile: [95]
      end
    end
