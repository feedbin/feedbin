class RecreateImages < ActiveRecord::Migration[8.1]
  def change
    create_table :images do |t|
      ...
      # Physical order matches databases built by the pre-squash per-column
      # migrations, so structure.sql dumps identically from either path.
      t.text :variant,             null: false
      t.uuid :original_fingerprint
    end
