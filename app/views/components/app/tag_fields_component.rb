      ...
      div class: "#{@tag_editor.tags.present? ? "mt-6" : ""}" do
        @tag_editor.tags.each do |tag|
          div do
            ...
            label_tag "tag_id[#{tag.id}]", class: "cursor-pointer group block flex items-center border rounded-lg mt-2 py-3 px-4 gap-3 select-none transition-[border,box_shadow,background] duration-200 peer-checked:border-blue-600 peer-checked:shadow-selected pointer-fine:hover:bg-100" do
