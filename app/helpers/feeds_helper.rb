module FeedsHelper
  ...
  # collections arrive includes(*Feed::ICON_PRELOADS), so this costs no query.
  def sidebar_feeds_cache_key(feeds)
    [feeds, feeds.map(&:title), feeds.map(&:site_favicon), "v10"]
  end
  ...
  def sidebar_tags_cache_key(tags)
    ...
    [tags, tags.map(&:user_feeds), feeds.map(&:title), feeds.map(&:site_favicon), "v16"]
  end
end
