...
import { afterAnimation, afterTransition } from "helpers"
...
export default class extends Controller {
  ...
  connect() {
    this.boundResize = () => this.goToPanel(this.stepValue, false)
    window.visualViewport.addEventListener("resize", this.boundResize)

    this.dialog = this.element.closest("dialog")
    if (this.dialog) {
      // the dialog is server rendered open so it is visible at first paint,
      // removing the attribute allows reopening it as a modal without
  ...
  close() {
    if (this.dialog) {
      this.dialog.dataset.closing = "true"
      afterAnimation(this.dialog, true, () => {
        this.dialog.close()
      })
    }
  }
