class ImageCopy
  ...
  def perform(entry_id)
    ...
    if entry.image && entry.image["processed_url"]
      url = copy(entry.image["processed_url"])
      image = entry.image
    ...
    if entry.data && entry.data["itunes_image_processed"]
      url = copy(entry.data["itunes_image_processed"], "-itunes")
      data = entry.data
  ...
  def copy(url, append = '')
    ...
    S3_POOL.with do |connection|
      connection.copy_object(ENV['AWS_S3_BUCKET'], source_object_name, ENV['AWS_S3_BUCKET_NEW'], destination_object_name, options)
    end
