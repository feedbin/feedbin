...
class ImageGarbageCollector
  ...
  def perform(entry_ids)
    ...
    # pluck: the rows are about to be deleted and only two values are
    # needed. transpose gives one array per column.
    rows = Image.entry_owned.where(provider_id: entry_ids).pluck(:id, :storage_path)
    return if rows.empty?

    ids, storage_paths = rows.transpose

    Image.where(id: ids).delete_all

    SweepStoredImages.perform_in(SWEEP_DELAY, storage_paths.uniq)
