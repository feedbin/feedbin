class PagesEntriesController < ApplicationController
  def index
    ...
    if view_mode == "view_all"
      @entries = pagination_anchor(@feed.entries, column: Entry.arel_table[:id]).includes(feed: Feed::ICON_PRELOADS).page(params[:page]).order("created_at DESC").entries_list
      @page_query = @entries
      ...
      @entries = Entry.in_order_of(:id, entry_ids).where(id: entry_ids).includes(feed: Feed::ICON_PRELOADS).entries_list
      ...
      @entries = Entry.where(id: unread_entries).includes(feed: Feed::ICON_PRELOADS).order("created_at DESC").entries_list
