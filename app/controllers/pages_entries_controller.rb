class PagesEntriesController < ApplicationController
  def index
    ...
    if view_mode == "view_all"
      @entries = pagination_anchor(@feed.entries, column: Entry.arel_table[:id]).includes(feed: [:favicon, :icon_image_record]).page(params[:page]).order("created_at DESC").entries_list
      @page_query = @entries
      ...
      @entries = Entry.in_order_of(:id, entry_ids).where(id: entry_ids).includes(feed: [:favicon, :icon_image_record]).entries_list
      ...
      @entries = Entry.where(id: unread_entries).includes(feed: [:favicon, :icon_image_record]).order("created_at DESC").entries_list
