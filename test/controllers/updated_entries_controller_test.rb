...
class UpdatedEntriesControllerTest < ActionController::TestCase
  ...
  test "should preload the preview image the entry cache key reads" do
    ...
    assert_operator images.count, :<=, 4, "flat images queries for the whole page (owned_image_records carries the preview, link preview and entry_icon rows): #{images.count}"
  end
end
