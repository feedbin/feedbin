...
class ActionResultsQueryCountTest < ActiveSupport::TestCase
  ...
  # a tweet or micropost whose link_preview? is true, and that gate reads the
  # link image row, which index_pair seeds.
  ...
  def index_pair
    ...
    # A fresh load: `linked` memoized its micropost inside harvest_embeds, on
    # create and again on update!, before seed_image wrote the link row, so
    # its @link_image is frozen at nil. The render loads entries fresh.
    assert Entry.find(linked.id).micropost&.link_preview?, "the render must reach link_image for this test to mean anything"
