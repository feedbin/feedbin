...
class EntryDeleterTestTest < ActiveSupport::TestCase
  ...
  test "should remove ids from created_at cache" do
    ...
    assert_difference -> {$redis[:sorted_entries].zcard(key_created_at)}, -removed_count do
      EntryDeleter.new().perform(@feed.id)
    end
  end
  ...
  test "should remove ids from published cache" do
    ...
    assert_difference -> {$redis[:sorted_entries].zcard(key_published)}, -removed_count do
      EntryDeleter.new().perform(@feed.id)
    end
  end
