...
class ImageGarbageCollectorTest < ActiveSupport::TestCase
  ...
  # The collector deletes rows and hands the paths on. Whether an
  ...
  test "deletes the rows and schedules a sweep for their paths" do
    ...
    assert_equal 1, scheduled_sweep["args"].size, "the sweep takes paths only"
    assert_equal [row_one.storage_path, row_two.storage_path].sort, scheduled_sweep["args"].first.sort
  end
